...
def get_checksum():
    # Part One
    return sum([max(spreadsheet[row]) - min(spreadsheet[row]) for row in range(len(spreadsheet))])


def get_evenly_divisible_numbers():
    # Part Two
    return sum([divisible_score_for_row(row) for row in range(len(spreadsheet))])


def divisible_score_for_row(row):
    """
    For a specified row in the spreadsheet, return the result of the first two numbers
    that are evenly divisible, or zero if there is none.
    :param row: The row in the spreadsheet.
    :return: The result of the division, or zero.
    """
    for ix, x in enumerate(spreadsheet[row]):
        for iy, y in enumerate(spreadsheet[row]):
            if ix == iy:
                continue
            if x % y == 0:
                return int(x / y)
...
print(get_checksum())
print(get_evenly_divisible_numbers())
